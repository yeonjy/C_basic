...
int main_pointer(void)
{
	...
	// [민수] : 301호
	// 각 문 앞에 '암호' 가 걸려 있음
	int 철수 = 1;  // 암호가 각각 1, 2, 3 이 되는 것
	int 영희 = 2;
	int 민수 = 3;

	printf("철수네 주소 : %d, 암호 : %d\n", &철수, 철수);                  // '철수'라는 변수가 저장된 메모리 공간의 주소 표현 -> &d
	...
	*스파이 = *스파이 - 2;
	...
	ptr[1] = 200;
	ptr[2] = 300;
	// ptr과 arr은 아예 같은 거라고 볼 수 있음
	for (int i = 0; i < 3; i++)
	{
		printf("배열 arr[%d] 의 값 : %d\n", i, arr[i]);                 // 100, 200, 300
		printf("배열 arr[%d]의 값 : %d\n", i, *(arr + i));                // arr의 첫째항으로부터 i개 뒤의 항 의미    //arr[i] 와 *(arr + i) 는 완전히 똑같다고 볼 수 있음
	}
	for (int i = 0; i < 3; i++)
	{
	...
	for (int i = 0; i < 3; i++)
	{
	 ...
	 // scanf 에서 &num 과 같이 &을 사용하는 이유 알 수 있음 -> num 자체의 값을 바꾸기 위함
